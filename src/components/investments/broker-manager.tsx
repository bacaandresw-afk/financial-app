  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        {brokers.length === 0 ? (
          <p className="text-sm text-muted-foreground">{t.investments.brokerManager.empty}</p>
        ) : (
  ...
  return (
    <form action={formAction} className="flex items-start gap-3">
      <div className="flex-1 space-y-1.5">
        <Label htmlFor="new-broker-name">{t.investments.brokerManager.newBrokerLabel}</Label>
        <Input
          id="new-broker-name"
          name="name"
          placeholder={t.investments.brokerManager.namePlaceholder}
          required
        />
        <FieldError>{state.error}</FieldError>
      </div>
      <Button type="submit" disabled={pending} className="mt-6">
        {pending ? t.investments.brokerManager.adding : t.investments.brokerManager.addBroker}
      </Button>
    </form>
  );
}
      ...
      <div className="rounded-lg border border-border p-3">
        <form action={updateFormAction} className="flex items-start gap-3">
          ...
          <Button type="submit" size="sm" disabled={updatePending}>
            {updatePending ? t.investments.brokerManager.saving : t.investments.brokerManager.save}
          </Button>
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-1">
          <Button
            ...
            aria-label={t.investments.brokerManager.rename}
          >
          ...
          <form action={deleteFormAction}>
            ...
            <Button
              ...
              aria-label={t.investments.brokerManager.delete}
              onClick={(e) => {
                if (!confirm(t.investments.brokerManager.confirmDelete(broker.name))) {
                  e.preventDefault();
                }
              }}
            >
